function M1 = analyzeSingleEvokedApr3_lastsweep(filename1, current_injected1)
%function ver. of singleEVOKED.m. This script finds the firing properties of the FIRST AP in the last sweep detected
...
%TODO: find why code is detecting second peak in test_spike for higehst
%current level.
...
for a= totalsweeps: totalsweeps %analyze the sweep that invokes the first AP in cell. change this to totalsweeps:totalsweeps to analyze 1st ap of last sweep
        ...
        if(isempty(all_dV_filtered))
            ...
            if(numel(pks_in_trace)>=1)
                ...
                if(numel(negative_slope)>=1)
                    ...
                    else %if more than 1 spike observed
                        %slopedecreasept = pks_in_trace(2)-risingDuration;
                        secondthresh = find(f > threshold_voltage); %second threshold
                        slopedecreasept = maintroughloc + secondthresh(1);
                    end
                end
