%This script finds the firing properties of the FIRST AP  detected in the
%LAST SWEEP from each cell in the directory, and exports the summary table as an excel
%file.
...
filenameExcelDoc = strcat('apr7_shrn_table_ls.xlsx');
...
for n=1:size(file_names,2)
   ...
   filename = string(file_names{n});
    disp([int2str(n) '. Working on: ' filename{:}])
         ...
         M1= analyzeSingleEvokedApr3_lastsweep(filename, current_injected);
         T1 = [T1; M1];
