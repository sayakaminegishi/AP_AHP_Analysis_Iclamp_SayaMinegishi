function M1 = analyzeSingleEvokedApr8(filename1, current_injected1, last_only)
...
if last_only,
    loop_vector = totalsweeps;
else,
    loop_vector = 1:totalsweeps;
end;

for a= loop_vector %analyze the sweep that invokes the first two APs in cell. change this to totalsweeps:totalsweeps to analyze 1st ap of last sweep
    ...
    if(apFoundInCell == 0)
        ...
        if(isempty(all_dV_filtered))
            error = "No action potentials identified in this trace (do we see this).";
